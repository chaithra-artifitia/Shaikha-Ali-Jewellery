...
const SubscriptionExpireSoon = ({ subscriptionExpireDate }) => {
    const formattedDate = subscriptionExpireDate
        ? new Date(subscriptionExpireDate).toLocaleDateString("en-GB", {
            day: "2-digit",
            month: "short",
            year: "numeric",
        })
    ...
    return (
        <div className="flex items-center justify-center px-4 py-8">
            <div className="relative w-full text-center">
                ...
                {/* Error message */}
                <h1 className="text-mid_red text-[2.5rem] font-bold mb-4">
                    Your Subscription Expiring Soon
                </h1>

                {/* Subtext */}
                <div className="text-white mb-4 font-medium">
                    <p className="text-xl">Your subscription will be <span className='text-light_blue'> expiring on {formattedDate}</span>.</p>
                    <p className="text-xl">Please subscribe to continue our service</p>
                </div>
...
export default SubscriptionExpireSoon;
